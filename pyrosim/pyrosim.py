...
def Send_Cube(name="default",pos=[0,0,0],size=[1,1,1], rgba: list[float] = [1, 1, 1, 1]):
    ...
    if filetype == SDF_FILETYPE:
        ...
        link = LINK_URDF(name,pos,size, rgba)
...
def Send_Joint(name,parent,child,type,position, joint_axis: str):

    joint = JOINT(name,parent,child,type,position)

    joint.Save(f, joint_axis)
